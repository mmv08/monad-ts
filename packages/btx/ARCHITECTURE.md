...
- The decoder makes one owned copy of the wire bytes, including for Node `Buffer` inputs. Components are disjoint views into that copy and share its backing buffer; later input changes cannot alter the ciphertext.
...
Each seed attempt consumes 16 bytes. The proof nonce uses Noble's BLS12-381 `randomSecretKey` helper, which maps 48 random bytes to a scalar in `[1, q)`. The test-key generator uses the same helper for its default trapdoor. Neither call uses the PDF's 64-byte wide reduction; `r` and the proof challenge still use it.

Vector tests pass each stored seed and nonce scalar directly to `encryptWithRandom`. They compare the encryption key and complete ciphertext against the stored file, then check the recovered plaintext and seed. A null `paddedLength` records that the caller omitted padding, so replay exercises default selection. Intermediate transcript values stay in the independent Rust known-answer tests rather than repeating across self-generated fixtures.
